...
public:
    ...
    void addRandomParticles(unsigned int count) {
        for(uint i=0; i<count; ++i) {
            glm::vec2 positionRandom = glm::linearRand(glm::vec2(-1.f,-1.f), glm::vec2(1.f,1.f));
            addParticle(positionRandom, 1.f, glm::vec3(0.f, 0.5f, 0.4f));
        }
    }
...
//    int newParticules = 10;
    ...
    while(!done) {
        ...
        // Rendu
        renderer.clear();
        //particleManager.addCircleParticles(0.5f, newParticules);
//        particleManager.addRandomParticles(10);
//        particleCount += newParticules;
