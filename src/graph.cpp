...
//    BrakeForce2D brakeForce(0.0001);
    ...
    // Graph Hook Force
    GraphHookForce2D graphHookForce(3, 0.15);
    // Graph Brake Force
    GraphBrakeForce2D graphBrakeForce(0.01);
    ...
    uint32_t diskFactor = 4;
    ParticleGraph particleGraph = createString(glm::vec2(-0.4,0), glm::vec2(0.4,0), glm::vec3(.5,1,.5), diskFactor, particleManager);

    graphHookForce.setGraph(&particleGraph);
    graphBrakeForce.setGraph(&particleGraph);
...
//        particleManager.drawParticles(renderer);
        particleManager.drawParticleGraph(particleGraph, renderer);
...
//        leapfrogSolver.solve(particleManager, dt);
        ...
        graphHookForce.apply(particleManager);
        graphBrakeForce.apply(particleManager);
        leapfrogSolver.solve(particleManager, dt);
